...
# n8n UI → Workflows → Import from file → workflows/loan-officer-assistant.json
...
```
workflows/                    # n8n workflow JSON exports (import via n8n UI)
...
- SQL text search (LIKE) is used for complaints/transcripts; the vector embedding columns exist but are for Denodo-native semantic search if the MCP server supports it
